...
* `switch_controller_source_ip` - Source IP address used in FortiLink over L3 connections. (`ver >= 6.6.0`)
* `mode` - Addressing mode (static, DHCP, PPPoE).
* `client_options` - DHCP client options. The structure of `client_options` block is documented below. (`ver >= 6.4.0`)
...
* `dhcp_relay_interface_select_method` - Specify how to select outgoing interface to reach server. (`ver 6.2.6,6.4.2,6.6.0`)
* `dhcp_relay_interface` - Specify outgoing interface to reach server. (`ver 6.2.6,6.4.2,6.6.0`)
...
* `scan_botnet_connections` - Enable monitoring or blocking connections to Botnet servers through this interface. (`ver <= 6.2.0`)
...
* `vlan_protocol` - Ethernet protocol of VLAN. (`ver >= 6.4.0`)
...
* `managed_device` - Available when FortiLink is enabled, used for managed devices through FortiLink interface. The structure of `managed_device` block is documented below. (`ver <= 6.2.0`)
...
* `captive_portal` - Enable/disable captive portal. (`ver <= 6.2.0`)
...
* `device_identification_active_scan` - Enable/disable active gathering of device identity information about the devices on the network connected to this interface. (`ver <= 6.2.0`)
* `device_access_list` - Device access list. (`ver <= 6.2.0`)
* `device_netscan` - Enable/disable inclusion of devices detected on this interface in network vulnerability scans. (`ver <= 6.2.0`)
...
* `fortiheartbeat` - Enable/disable FortiHeartBeat (FortiTelemetry on GUI). (`ver <= 6.2.0`)
* `broadcast_forticlient_discovery` - Enable/disable broadcasting FortiClient discovery messages.
* `endpoint_compliance` - Enable/disable endpoint compliance enforcement. (`ver <= 6.2.0`)
...
* `measured_upstream_bandwidth` - Measured upstream bandwidth (kbps).  (`ver >= 6.4.0`)
* `measured_downstream_bandwidth` - Measured downstream bandwidth (kbps).  (`ver >= 6.4.0`)
* `bandwidth_measure_time` - Bandwidth measure time  (`ver >= 6.4.0`)
* `monitor_bandwidth` - Enable monitoring bandwidth on this interface. (`ver >= 6.4.0`)
...
* `fortilink_stacking` - Enable/disable FortiLink switch-stacking on this interface. (`ver <= 6.4.2`)
* `fortilink_neighbor_detect` - Protocol for FortiGate neighbor discovery.
* `ip_managed_by_fortiipam` - Enable/disable automatic IP address assignment of this interface by FortiIPAM. (`ver >= 6.4.0`)
* `managed_subnetwork_size` - Number of IP addresses to be allocated by FortiIPAM and used by this FortiGate unit's DHCP server settings. (`ver >= 6.4.0`)
...
* `switch_controller_mgmt_vlan` - VLAN to use for FortiLink management purposes. (`ver >= 6.4.0`)
...
* `switch_controller_nac` - Integrated NAC settings for managed FortiSwitch. (`ver >= 6.4.0`)
* `switch_controller_feature` - Interface's purpose when assigning traffic (read only). (`ver >= 6.4.0`)
* `switch_controller_iot_scanning` - Enable/disable managed FortiSwitch IoT scanning. (`ver >= 6.6.0`)
* `swc_vlan` - Creation status for switch-controller VLANs. (`ver >= 6.4.0`)
* `swc_first_create` - Initial create for switch-controller VLANs. (`ver >= 6.6.0`)
...
The `client_options` block supports (`ver >= 6.4.0`):
...
The `managed_device` block supports (`ver <= 6.2.0`):
...
* `icmp6_send_redirect` - Enable/disable sending of ICMPv6 redirects. (`ver >= 6.6.0`)
...
* `unique_autoconf_addr` - Enable/disable unique auto config address. (`ver >= 6.4.2`)
* `interface_identifier` - IPv6 interface identifier. (`ver >= 6.4.2`)
* `ip6_prefix_mode` - Assigning a prefix from DHCP or RA. (`ver >= 6.6.0`)
...
* `cli_conn6_status` - CLI IPv6 connection status. (`ver >= 6.6.0`)
