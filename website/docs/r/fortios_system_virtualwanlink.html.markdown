...
* `neighbor_hold_down` - Enable/disable hold switching from the secondary neighbor to the primary neighbor. (`ver <= 6.4.0`)
* `neighbor_hold_down_time` - Waiting period in seconds when switching from the secondary neighbor to the primary neighbor when hold-down is disabled. (0 - 10000000, default = 0). (`ver <= 6.4.0`)
* `neighbor_hold_boot_time` - Waiting period in seconds when switching from the primary neighbor to the secondary neighbor from the neighbor start. (0 - 10000000, default = 0). (`ver <= 6.4.0`)
...
* `zone` - Configure SD-WAN zones. The structure of `zone` block is documented below. (`ver 6.2.6`)
...
* `neighbor` - Create SD-WAN neighbor from BGP neighbor table to control route advertisements according to SLA status. The structure of `neighbor` block is documented below. (`ver <= 6.4.0`)
...
The `zone` block supports (`ver 6.2.6`):
...
* `system_dns` - Enable/disable system DNS as the probe server. (`ver 6.4.0`)
...
* `ha_priority` - HA election priority (1 - 50). (`ver <= 6.4.0`)
...
* `dns_request_domain` - Fully qualified domain name to resolve for the DNS probe. (`ver 6.4.0`)
* `interval` - Status check interval, or the time between attempting to connect to the server (1 - 3600 sec, default = 5).
* `probe_timeout` - Time to wait before a probe packet is considered lost (500 - 5000 msec, default = 500). (`ver <= 6.4.0`)
...
* `diffservcode` - Differentiated services code point (DSCP) in the IP header of the probe packet. (`ver <= 6.4.0`)
...
The `neighbor` block supports (`ver <= 6.4.0`):
...
* `input_device_negate` - Enable/disable negation of input device match. (`ver <= 6.4.0`)
* `mode` - Control how the priority rule sets the priority of interfaces in the SD-WAN.
* `role` - Service role to work with neighbor. (`ver <= 6.4.0`)
* `standalone_action` - Enable/disable service when selected neighbor role is standalone while service role is not standalone. (`ver <= 6.4.0`)
* `quality_link` - Quality grade.
* `member` - Member sequence number. (`ver 6.2.0,6.2.4`)
...
* `internet_service_name` - Internet service name list. The structure of `internet_service_name` block is documented below. (`ver 6.4.0`)
...
* `internet_service_app_ctrl` - Application control based Internet Service ID list. The structure of `internet_service_app_ctrl` block is documented below. (`ver <= 6.4.0`)
* `internet_service_app_ctrl_group` - Application control based Internet Service group list. The structure of `internet_service_app_ctrl_group` block is documented below. (`ver <= 6.4.0`)
* `internet_service_ctrl` - Control-based Internet Service ID list. The structure of `internet_service_ctrl` block is documented below. (`ver <= 6.2.0`)
* `internet_service_ctrl_group` - Control-based Internet Service group list. The structure of `internet_service_ctrl_group` block is documented below. (`ver <= 6.2.0`)
...
* `default` - Enable/disable use of SD-WAN as default service. (`ver <= 6.4.0`)
* `sla_compare_method` - Method to compare SLA value for sla and load balance mode.  (`ver <= 6.4.0`)
...
The `internet_service_app_ctrl` block supports (`ver <= 6.4.0`):

* `id` - Application control based Internet Service ID.

The `internet_service_app_ctrl_group` block supports (`ver <= 6.4.0`):

* `name` - Application control based Internet Service group name.

The `internet_service_ctrl` block supports (`ver <= 6.2.0`):

* `id` - Control-based Internet Service ID.

The `internet_service_ctrl_group` block supports (`ver <= 6.2.0`):
