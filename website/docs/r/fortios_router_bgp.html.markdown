...
* `multipath_recursive_distance` - Enable/disable use of recursive distance to select multipath. (`ver >= 6.4.2`)
* `recursive_next_hop` - Enable/disable recursive resolution of next-hop using BGP route. (`ver >= 6.6.0`)
...
* `vrf_leak` - BGP VRF leaking table. The structure of `vrf_leak` block is documented below. (`ver >= 6.4.0`)
...
* `next_hop_self_rr` - Enable/disable setting nexthop's address to interface's IPv4 address for route-reflector routes. (`ver >= 6.6.0`)
* `next_hop_self_rr6` - Enable/disable setting nexthop's address to interface's IPv6 address for route-reflector routes. (`ver >= 6.6.0`)
...
The `vrf_leak` block supports (`ver >= 6.4.0`):
