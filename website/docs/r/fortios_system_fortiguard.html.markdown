...
* `service_account_id` - Service account ID. (`ver <= 6.2.0`)
...
* `anycast_sdns_server_ip` - IP address of the FortiGuard anycast DNS rating server. (`ver >= 6.4.2`)
* `anycast_sdns_server_port` - Port to connect to on the FortiGuard anycast DNS rating server. (`ver >= 6.4.2`)
* `sdns_options` - Customization options for the FortiGuard DNS service. (`ver >= 6.4.0`)
