...
* `system_diagnostics` - Enable/disable permission to run system diagnostic commands. (`ver >= 6.4.0`)
...
* `spamfilter` - AntiSpam filter and settings. (`ver <= 6.2.0`)
* `data_loss_prevention` - DLP profiles and settings.
* `file_filter` - File-filter profiles and settings. (`ver >= 6.4.2`)
