...
* `srcaddr6` - Source IPv6 address name and address group names. The structure of `srcaddr6` block is documented below. (`ver >= 6.4.0`)
* `dstaddr6` - Destination IPv6 address name and address group names. The structure of `dstaddr6` block is documented below. (`ver >= 6.4.0`)
...
* `src_vendor_mac` - Vendor MAC source ID. The structure of `src_vendor_mac` block is documented below. (`ver >= 6.4.0`)
...
* `learning_mode` - Enable to allow everything, but log all of the meaningful data for security information gathering. A learning report will be generated. (`ver <= 6.2.0`)
...
* `geoip_match` - Match geography address based either on its physical location or registered location. (`ver >= 6.4.0`)
...
* `application` - Application ID list. The structure of `application` block is documented below. (`ver <= 6.4.0`)
* `app_category` - Application category ID list. The structure of `app_category` block is documented below. (`ver <= 6.4.0`)
* `url_category` - URL category ID list. The structure of `url_category` block is documented below. (`ver <= 6.4.0`)
* `app_group` - Application group names. The structure of `app_group` block is documented below. (`ver <= 6.4.0`)
...
* `poolname6` - IPv6 pool names. The structure of `poolname6` block is documented below. (`ver >= 6.4.0`)
...
* `scan_botnet_connections` - Block or monitor connections to Botnet servers or disable Botnet scanning. (`ver <= 6.2.0`)
...
The `srcaddr6` block supports (`ver >= 6.4.0`):

* `name` - Address name.

The `dstaddr6` block supports (`ver >= 6.4.0`):
...
The `src_vendor_mac` block supports (`ver >= 6.4.0`):
...
The `application` block supports (`ver <= 6.4.0`):

* `id` - Application IDs.

The `app_category` block supports (`ver <= 6.4.0`):

* `id` - Category IDs.

The `url_category` block supports (`ver <= 6.4.0`):

* `id` - URL category ID.

The `app_group` block supports (`ver <= 6.4.0`):
...
The `poolname6` block supports (`ver >= 6.4.0`):
