...
* `virtual_host_only` - Enable/disable enforcement of virtual host method for SSL-VPN client access. (`ver >= 6.4.2`)
* `radius_server` - RADIUS server associated with realm. (`ver >= 6.4.0`)
* `nas_ip` - IP address used as a NAS-IP to communicate with the RADIUS server. (`ver >= 6.4.0`)
* `radius_port` - RADIUS service port number (0 - 65535, 0 means user.radius.radius-port). (`ver >= 6.4.2`)
