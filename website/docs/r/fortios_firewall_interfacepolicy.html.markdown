...
* `address_type` - Policy address type (IPv4 or IPv6). (`ver <= 6.2.0`)
...
* `spamfilter_profile_status` - Enable/disable antispam. (`ver <= 6.2.0`)
* `spamfilter_profile` - Antispam profile. (`ver <= 6.2.0`)
* `dlp_sensor_status` - Enable/disable DLP.
* `dlp_sensor` - DLP sensor name.
* `scan_botnet_connections` - Enable/disable scanning for connections to Botnet servers. (`ver <= 6.2.0`)
* `label` - Label. (`ver <= 6.2.0`)
